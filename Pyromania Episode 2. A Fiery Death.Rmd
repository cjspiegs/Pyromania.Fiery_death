...
```{r}
# Time 0
index.T0.df=read.csv("/Users/Cody1/Desktop/Projects/Pyromania/My_pyromania/EEMS/Indexresults/T0_indices.csv")
index.T0.df$Time.point="T0"
names(index.T0.df)[6]="Humification.Index"
index.T0.df=index.T0.df[-c(7:11,13,14)]

#Time 1
index.T1.df=read.csv("/Users/Cody1/Desktop/Projects/Pyromania/My_pyromania/EEMS/Indexresults/T1_indices.csv")
index.T1.df$Time.point="T1"
index.T1.df=index.T1.df[-c(7:11,13,14)]

#Time 2
index.T2.df=read.csv("/Users/Cody1/Desktop/Projects/Pyromania/My_pyromania/EEMS/Indexresults/T2_indices.csv")
index.T2.df$Time.point="T2"
index.T2.df=index.T2.df[-c(7:11,13,14)]

#Time 3
index.T3.df=read.csv("/Users/Cody1/Desktop/Projects/Pyromania/My_pyromania/EEMS/Indexresults/T3_indices.csv")
index.T3.df$Time.point="T3"
index.T3.df=index.T3.df[-c(7:11,13,14)]


#Time 4
index.T4.df=read.csv("/Users/Cody1/Desktop/Projects/Pyromania/My_pyromania/EEMS/Indexresults/T4_indicies.csv")
...
burned.m1=gam(percent.change.DOC ~ Type +  s(plant.mass..g, by=Type), data = degra_t1.burned.df, method = "REML",select= T)
summary(burned.m1)

unburned.m1=gam(percent.change.DOC ~ Type +  s(plant.mass..g, by=Type), data = degra_t1.unburned.df, method = "REML",select= T)
summary(unburned.m1)
...
photo.df2 <- transform(photo.df, oTreatment = ordered(Treatment))
levels(photo.df2$Treatment)

burned.gam3=gam(percent.change.DOC ~ Type +  s(plant.mass..g, by=Type), data = degra_t1.burned.df, method = "REML",select= T)

gam3_diff=difference_smooths(burned.gam3, smooth = "s(plant.mass..g)", group_means=TRUE)
gam3_diff
xp=predict(burned.gam3, newdata= degra_t1.burned.df, type = "lpmatrix")


## which cols of xp relate to splines of interest?
c1 <- grepl('UV-only', colnames(xp))
c2 <- grepl('micorbes-only', colnames(xp))
## which rows of xp relate to sites of interest?
r1 <- with(degra_t1.burned.df, Type == 'UV-only')
r2 <- with(degra_t1.burned.df, Type == 'microbes-only')
...
dif <- X %*% coef(burned.gam3)

se <- sqrt(rowSums((X %*% vcov(burned.gam3)) * X))
crit <- qt(.975, df.residual(burned.gam3))
upr <- dif + (crit * se)
lwr <- dif - (crit * se)
...
comp1 <- smooth_diff(burned.gam3, degra_t1.burned.df, 'UV-only', 'microbes-only', 'Type')
comp2 <- smooth_diff(burned.gam3, degra_t1.burned.df, 'UV-only', 'UV and microbes', 'Type')
comp3 <- smooth_diff(burned.gam3, degra_t1.burned.df, 'microbes-only', 'UV and microbes', 'Type')
comp <- cbind(plant.mass..g = seq(0, 400, length = 15),
              rbind(comp1, comp2, comp3))

ggplot(comp, aes(x = plant.mass..g, y = diff, group = pair)) +
    geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.2) +
    geom_line() +
    facet_wrap(~ pair, ncol = 2) +
    coord_cartesian(ylim = c(-30,30)) +
    labs(x = NULL, y = 'Difference in DOC trend')
...
#compare difference between effect on DOC loss


m1.loss.perc=gam(percent.change.DOC ~ Type +s(plant.mass..g, by= Type), data=degra.df, method="REML", select= T, subset= Time.point=="T1")
gam.check(m1.loss.perc, rep=1000)
draw(m1.loss.perc)
m1.loss.aov=anova(m1.loss.perc)
m1.loss.aov
summary(m1.loss.perc)
TukeyHSD(m1.loss.aov)
plot_difference(m1.loss.perc)




degra.df2 <- transform(degra.df, oTreatment = ordered(Treatment))
levels(degra.df2$Treatment)

gam3 <- gam(percent.change.DOC ~ oType + s(plant.mass..g, by = oType),
            data = degra.df2, method = "ML")

levels(degra.df2$oType)
summary(gam3)
...
```{r DOC model plots compiled}
fsdfsd
mean(microbe_t1.df[microbe_t1.df$plant.mass..g<="75" & microbe_t1.df$plant.mass..g>="275",]$percent.change.DOC)

mean(microbe_t1.df[microbe_t1.df$plant.mass..g>="150" & microbe_t1.df$plant.mass..g<="250",]$percent.change.DOC)
...
ggplot(degra_t1.df, aes(x= plant.mass..g, y=percent.change.DOC, linetype=Treatment, color=Type, fill=Type))+
  geom_point()+
 scale_color_manual(values = c("gold", "magenta", "cornflowerblue")) + 
   scale_fill_manual(values = c("gold", "magenta", "cornflowerblue")) +
  geom_smooth(method = "gam", formula = y~s(x))+
   geom_hline(yintercept=0, linetype="longdash", color = "gray") +
  facet_grid(~Treatment)+
  Fig.formatting.boxplot

ggplot(degra_t1.df, aes(x= plant.mass..g, y=percent.change.DOC, linetype=Treatment, color=Treatment, fill=Treatment))+
...
% change DOC across SUVA (relate composition with decomposition)
```{r}
###Decomposition across SUVA for photo
m1.suva.photo=gam(percent.change.DOC ~ Treatment +s(SUVA, by= Treatment), data=photo.df, method="REML", select= T, subset= Time.point=="T1")

m2.suva.photo=gam(percent.change.DOC ~ Treatment +s(SUVA), data=photo.df, method="REML", select= T, subset= Time.point=="T1")

m3.suva.photo=gam(percent.change.DOC ~ s(SUVA), data=photo.df, method="REML", select= T, subset= Time.point=="T1")

photo.suva.AIC=AIC(m1.suva.photo, m2.suva.photo, m3.suva.photo)
photo.suva.AIC
photo.suva.anova=anova(m1.suva.photo)
photo.suva.anova

# global smooth

photo.suva.plot=plot_smooths(
  model = m3.suva.photo,
  series = SUVA
) + 
  geom_point(data=photo.df[(photo.df$Time.point=="T1"),], 
             aes(x=SUVA, y=percent.change.DOC, color=Treatment)) +
   scale_color_manual(values = c("brown1", "mediumseagreen")) +
  ggtitle("UV-only") +
   geom_hline(yintercept=0, linetype="longdash", color = "gray") +
  ylab("Change DOC (%)") +
xlab(expression('SUVA'["254"]~'(mgC'~ L^-1~m^-1*')'))+

  Fig.formatting

ggplot(DOC.df, aes(x=DOC..mg.L, y=SUVA, fill=Treatment, color=Treatment))+
  geom_point()+
  geom_smooth(method = "gam", formula = y~s(x))+
...
###Decomposition across SUVA for microbes
m1.suva.microbes=gam(percent.change.DOC ~ Treatment +s(SUVA, by= Treatment), data=microbe.df, method="REML", select= T, subset= Time.point=="T1")

m2.suva.microbes=gam(percent.change.DOC ~ Treatment +s(SUVA), data=microbe.df, method="REML", select= T, subset= Time.point=="T1")

m3.suva.microbes=gam(percent.change.DOC ~ s(SUVA), data=microbe.df, method="REML", select= T, subset= Time.point=="T1")

microbe.suva.AIC=AIC(m1.suva.microbes, m2.suva.microbes, m3.suva.microbes)
microbe.suva.AIC
microbe.suva.anova=anova(m1.suva.microbes)
summary(m3.suva.microbes)
# global smooth

microbe.suva.plot=plot_smooths(
  model = m3.suva.microbes,
  series = SUVA
) + 
  geom_point(data=microbe.df[(microbe.df$Time.point=="T1"),], 
             aes(x=SUVA, y=percent.change.DOC, color=Treatment)) +
...
microbe.suva.plot
  ###Decomposition across SUVA for microbes
m1.suva.total=gam(percent.change.DOC ~ Treatment +s(SUVA, by= Treatment), data=total.df, method="REML", select= T, subset= Time.point=="T1")

m2.suva.total=gam(percent.change.DOC ~ Treatment +s(SUVA), data=total.df, method="REML", select= T, subset= Time.point=="T1")

m3.suva.total=gam(percent.change.DOC ~ s(SUVA), data=total.df, method="REML", select= T, subset= Time.point=="T1")

total.suva.AIC=AIC(m1.suva.total, m2.suva.total, m3.suva.total)
total.suva.AIC
total.suva.anova=anova(m1.suva.total)

# global smooth

total.suva.plot=plot_smooths(
  model = m3.suva.total,
  series = SUVA
) + 
  geom_point(data=total.df[(total.df$Time.point=="T1"),], 
...
##combine AIC
mod.degra.suva.rep<-rep(c("UV-only: Treatment with by-factor smooth", 
              "UV-only: Treatment with global smooth", 
              "UV-only: global smooth",
              "Microbes-only: Treatment with by-factor smooth", 
              "Microbes-only: Treatment with global smooth", 
...
mod.degra.suva.column<- data.frame(mod.degra.suva.rep)
AIC.degra.suva<-bind_rows(photo.suva.AIC, microbe.suva.AIC, total.suva.AIC)
AIC.degra.suva.mod<-as.data.frame(cbind(mod.degra.suva.column, AIC.degra.suva))

names(AIC.degra.suva.mod)[1]="Model"
AIC.degra.suva.mod <- AIC.degra.suva.mod %>%
  # Creating an empty column:
  add_column(Variable= c("SUVA"), .before="Model")

write.csv(AIC.degra.suva.mod, "output/AIC models/AIC.degra.suva.csv")

#convert dataframe into a table for export
pdf("/Users/Cody1/Desktop/GitHub/Pyromania2/output/Codys.tables/degra.suva.mod.AIC.pdf", width = 6.5, height = 2.8)       # Export PDF
grid.table(AIC.degra.suva.mod, rows=NULL)
dev.off()
...
```{r}
####Parametric output
degra.suva.para.sums=as.data.frame(rbind(photo.suva.anova$pTerms.table, microbe.suva.anova$pTerms.table, total.suva.anova$pTerms.table))
degra.suva.para.sums
names(degra.suva.para.sums)[1]="df/edf"

#create empty column for ref.df
degra.suva.para.sums <- degra.suva.para.sums %>%
  # Creating an empty column:
  add_column(Model= c("UV-only Model: Treatment","Microbes-only Model: Treatment","UV and microbes Model: Treatment"), .before="df/edf")%>%
  add_column(Ref.df = "-", .after="df/edf")
...
#####Smooth output
degra.smooth.suva.sums=as.data.frame(rbind(photo.suva.anova$s.table, microbe.suva.anova$s.table, total.suva.anova$s.table))
names(degra.smooth.suva.sums)[1]="df/edf"



#create empty column for ref.df
degra.smooth.suva.sums <- degra.smooth.suva.sums %>%
  # Creating an empty column:
  add_column(Model= c("UV-only Model: burned smooth","UV-only Model: unburned smooth","Microbes-only Model: burned smooth","Microbes-only Model: unburned smooth","UV and microbes Model: burned smooth","UV and microbes Model: unburned smooth"), .before="df/edf")

#merge the dataframes
degra.suva.mod.allsums=rbind(degra.suva.para.sums,degra.smooth.suva.sums)

degra.suva.mod.allsums <- degra.suva.mod.allsums %>%
  # Creating an empty column:
  add_column(Variable= c("SUVA"), .before="Model")


write.csv(degra.suva.mod.allsums,"/Users/Cody1/Desktop/GitHub/Pyromania2/output/Codys.tables/degra.suva.mod.allsums.csv")

#convert dataframe into a table for export
pdf("/Users/Cody1/Desktop/GitHub/Pyromania2/output/Codys.tables/degra.suva.mod.allsums.pdf", width = 8.5, height = 2.8)       # Export PDF
grid.table(degra.suva.mod.allsums, rows=NULL)
dev.off()
...
degra.suva.mod.plot<-plot_grid(
photo.suva.plot+ theme(legend.position = "none"),
microbe.suva.plot+ theme(legend.position = "none"),
total.suva.plot+ theme(legend.position = "none"), extract.legend,
  rel_widths = c(8,8,8,3), ncol=4)

degra.suva.mod.plot

ggsave("figures/degra.suva.mod.plot.pdf", height=4, width=12)

```


% change DOC across HIX (relate composition with decomposition)
```{r}
###Decomposition across SUVA for photo
m1.hix.photo=gam(percent.change.DOC ~ Treatment +s(HIX, by= Treatment), data=photo.df, method="REML", select= T, subset= Time.point=="T1")

m2.hix.photo=gam(percent.change.DOC ~ Treatment +s(HIX), data=photo.df, method="REML", select= T, subset= Time.point=="T1")

m3.hix.photo=gam(percent.change.DOC ~ s(HIX), data=photo.df, method="REML", select= T, subset= Time.point=="T1")

photo.hix.AIC=AIC(m1.hix.photo, m2.hix.photo, m3.hix.photo)
photo.hix.AIC
photo.hix.anova=anova(m1.hix.photo)
photo.hix.anova

photo.hix.plot=plot_smooths(
  model = m2.hix.photo,
  series = HIX,
  comparison = Treatment
) + 
  geom_point(data=photo.df[(photo.df$Time.point=="T1"),], 
             aes(x=HIX, y=percent.change.DOC, color=Treatment)) +
   scale_color_manual(values = c("brown1", "mediumseagreen")) +
   scale_fill_manual(values = c("brown1", "mediumseagreen")) +
  ggtitle("UV-only") +
   geom_hline(yintercept=0, linetype="longdash", color = "gray") +
...
###Decomposition across HIX for microbes
m1.hix.microbes=gam(percent.change.DOC ~ Treatment +s(HIX, by= Treatment), data=microbe.df, method="REML", select= T, subset= Time.point=="T1")

m2.hix.microbes=gam(percent.change.DOC ~ Treatment +s(HIX), data=microbe.df, method="REML", select= T, subset= Time.point=="T1")

m3.hix.microbes=gam(percent.change.DOC ~ s(HIX), data=microbe.df, method="REML", select= T, subset= Time.point=="T1")

microbe.hix.AIC=AIC(m1.hix.microbes, m2.hix.microbes, m3.hix.microbes)
microbe.hix.anova=anova(m1.hix.microbes)

microbe.AIC
# global smooth
microbe.hix.plot=plot_smooths(
  model = m3.hix.microbes,
  series = HIX
) + 
  geom_point(data=microbe.df[(microbe.df$Time.point=="T1"),], 
             aes(x=HIX, y=percent.change.DOC, color=Treatment)) +
  ...
  ###Decomposition across HIX for microbes
m1.hix.total=gam(percent.change.DOC ~ Treatment +s(HIX, by= Treatment), data=total.df, method="REML", select= T, subset= Time.point=="T1")

m2.hix.total=gam(percent.change.DOC ~ Treatment +s(HIX), data=total.df, method="REML", select= T, subset= Time.point=="T1")

m3.hix.total=gam(percent.change.DOC ~ s(HIX), data=total.df, method="REML", select= T, subset= Time.point=="T1")

total.hix.AIC=AIC(m1.hix.total, m2.hix.total, m3.hix.total)
total.hix.anova=anova(m1.hix.total)
# global smooth
total.hix.plot=plot_smooths(
  model = m3.hix.total,
  series = HIX
) + 
  geom_point(data=total.df[(total.df$Time.point=="T1"),], 
...
##combine AIC
mod.degra.hix.rep<-rep(c("UV-only: Treatment with by-factor smooth", 
              "UV-only: Treatment with global smooth", 
              "UV-only: global smooth",
              "Microbes-only: Treatment with by-factor smooth", 
              "Microbes-only: Treatment with global smooth", 
...
mod.degra.hix.column<- data.frame(mod.degra.hix.rep)
AIC.degra.hix<-bind_rows(photo.hix.AIC, microbe.hix.AIC, total.hix.AIC)
AIC.degra.hix.mod<-as.data.frame(cbind(mod.degra.hix.column, AIC.degra.hix))

names(AIC.degra.hix.mod)[1]="Model"
AIC.degra.hix.mod <- AIC.degra.hix.mod %>%
  # Creating an empty column:
  add_column(Variable= c("HIX"), .before="Model")

write.csv(AIC.degra.hix.mod, "output/AIC models/AIC.degra.hix.csv")

#convert dataframe into a table for export
pdf("/Users/Cody1/Desktop/GitHub/Pyromania2/output/Codys.tables/degra.hix.mod.AIC.pdf", width = 6.5, height = 2.8)       # Export PDF
grid.table(AIC.degra.hix.mod, rows=NULL)
dev.off()
...
```{r}
####Parametric output
degra.hix.para.sums=as.data.frame(rbind(photo.hix.anova$pTerms.table, microbe.hix.anova$pTerms.table, total.hix.anova$pTerms.table))

names(degra.hix.para.sums)[1]="df/edf"

#create empty column for ref.df
degra.hix.para.sums <- degra.hix.para.sums %>%
  # Creating an empty column:
  add_column(Model= c("UV-only Model: Treatment","Microbes-only Model: Treatment","UV and microbes Model: Treatment"), .before="df/edf")%>%
  add_column(Ref.df = "-", .after="df/edf")
...
#####Smooth output
degra.smooth.hix.sums=as.data.frame(rbind(photo.hix.anova$s.table, microbe.hix.anova$s.table, total.hix.anova$s.table))
names(degra.smooth.hix.sums)[1]="df/edf"



#create empty column for ref.df
degra.smooth.hix.sums <- degra.smooth.hix.sums %>%
  # Creating an empty column:
  add_column(Model= c("UV-only Model: burned smooth","UV-only Model: unburned smooth","Microbes-only Model: burned smooth","Microbes-only Model: unburned smooth","UV and microbes Model: burned smooth","UV and microbes Model: unburned smooth"), .before="df/edf")

#merge the dataframes
degra.hix.mod.allsums=rbind(degra.hix.para.sums,degra.smooth.hix.sums)

degra.hix.mod.allsums <- degra.hix.mod.allsums %>%
  # Creating an empty column:
  add_column(Variable= c("HIX"), .before="Model")


write.csv(degra.hix.mod.allsums, "/Users/Cody1/Desktop/GitHub/Pyromania2/output/Codys.tables/degra.hix.mod.allsums.csv")

#convert dataframe into a table for export
pdf("/Users/Cody1/Desktop/GitHub/Pyromania2/output/Codys.tables/degra.hix.mod.allsums.pdf", width = 8.5, height = 2.8)       # Export PDF
grid.table(degra.hix.mod.allsums, rows=NULL)
dev.off()
...
degra.hix.mod.plot<-plot_grid(
photo.hix.plot+ theme(legend.position = "none"),
microbe.hix.plot+ theme(legend.position = "none"),
total.hix.plot+ theme(legend.position = "none"), extract.legend,
  rel_widths = c(8,8,8,3), ncol=4)

degra.hix.mod.plot

ggsave("figures/degra.hix.mod.plot.pdf", height=4, width=12)

```

% change DOC across Sr (relate composition with decomposition)
```{r}
###Decomposition across Sr for photo
m1.sr.photo=gam(percent.change.DOC ~ Treatment +s(Spectral.slope, by= Treatment), data=photo.df, method="REML", select= T, subset= Time.point=="T1")

m2.sr.photo=gam(percent.change.DOC ~ Treatment +s(Spectral.slope), data=photo.df, method="REML", select= T, subset= Time.point=="T1")

m3.sr.photo=gam(percent.change.DOC ~ s(Spectral.slope), data=photo.df, method="REML", select= T, subset= Time.point=="T1")

photo.sr.AIC=AIC(m1.sr.photo, m2.sr.photo, m3.sr.photo)
photo.sr.anova=anova(m1.sr.photo)
# global smooth
photo.sr.plot=plot_smooths(
  model = m3.sr.photo,
  series = Spectral.slope
) + 
  geom_point(data=photo.df[(photo.df$Time.point=="T1"),], 
             aes(x=Spectral.slope, y=percent.change.DOC, color=Treatment)) +
   scale_color_manual(values = c("brown1", "mediumseagreen")) +
  ggtitle("UV-only") +
   geom_hline(yintercept=0, linetype="longdash", color = "gray") +
  ylab("Change DOC (%)") +
   xlab(expression('S'[italic(R)]))+
  coord_cartesian(ylim = c(-30,0))+
  Fig.formatting
 

###Decomposition across Sr for microbes
m1.sr.microbes=gam(percent.change.DOC ~ Treatment +s(Spectral.slope, by= Treatment), data=microbe.df, method="REML", select= T, subset= Time.point=="T1")

m2.sr.microbes=gam(percent.change.DOC ~ Treatment +s(Spectral.slope), data=microbe.df, method="REML", select= T, subset= Time.point=="T1")

m3.sr.microbes=gam(percent.change.DOC ~ s(Spectral.slope), data=microbe.df, method="REML", select= T, subset= Time.point=="T1")

microbe.sr.AIC=AIC(m1.sr.microbes, m2.sr.microbes, m3.sr.microbes)
microbe.sr.anova=anova(m1.sr.microbes)
# global smooth
microbe.sr.plot=plot_smooths(
  model = m3.sr.microbes,
  series= Spectral.slope
) + 
  geom_point(data=microbe.df[(microbe.df$Time.point=="T1"),], 
             aes(x=Spectral.slope, y=percent.change.DOC, color=Treatment)) +
  ...
  ###Decomposition across Sr for microbes
m1.sr.total=gam(percent.change.DOC ~ Treatment +s(Spectral.slope, by= Treatment), data=total.df, method="REML", select= T, subset= Time.point=="T1")

m2.sr.total=gam(percent.change.DOC ~ Treatment +s(Spectral.slope), data=total.df, method="REML", select= T, subset= Time.point=="T1")

m3.sr.total=gam(percent.change.DOC ~ s(Spectral.slope), data=total.df, method="REML", select= T, subset= Time.point=="T1")

total.sr.AIC=AIC(m1.sr.total, m2.sr.total, m3.sr.total)
total.sr.anova=anova(m1.sr.total)
# global smooth
total.sr.plot=plot_smooths(
  model = m3.sr.total,
  series = Spectral.slope
) + 
  geom_point(data=total.df[(total.df$Time.point=="T1"),], 
...
```{r}
####Parametric output
degra.sr.para.sums=as.data.frame(rbind(photo.sr.anova$pTerms.table, microbe.sr.anova$pTerms.table, total.sr.anova$pTerms.table))

names(degra.sr.para.sums)[1]="df/edf"

#create empty column for ref.df
degra.sr.para.sums <- degra.sr.para.sums %>%
  # Creating an empty column:
  add_column(Model= c("UV-only Model: Treatment","Microbes-only Model: Treatment","UV and microbes Model: Treatment"), .before="df/edf")%>%
  add_column(Ref.df = "-", .after="df/edf")
...
#####Smooth output
degra.smooth.sr.sums=as.data.frame(rbind(photo.sr.anova$s.table, microbe.sr.anova$s.table, total.sr.anova$s.table))
names(degra.smooth.sr.sums)[1]="df/edf"



#create empty column for ref.df
degra.smooth.sr.sums <- degra.smooth.sr.sums %>%
  # Creating an empty column:
  add_column(Model= c("UV-only Model: s(plant.mass):burned","UV-only Model: s(plant.mass):unburned","Microbes-only Model: s(plant.mass):burned","Microbes-only Model: s(plant.mass):unburned","UV and microbes Model: s(plant.mass):burned","UV and microbes Model: s(plant.mass):unburned"), .before="df/edf")

#merge the dataframes
degra.sr.mod.allsums=rbind(degra.sr.para.sums,degra.smooth.sr.sums)
rownames(degra.sr.mod.allsums)<-c(1:9)

write.csv(degra.sr.mod.allsums, "/Users/Cody1/Desktop/GitHub/Pyromania2/output/Codys.tables/degra.sr.mod.allsums.csv")

#convert dataframe into a table for export
pdf("/Users/Cody1/Desktop/GitHub/Pyromania2/output/Codys.tables/degra.sr.mod.allsums.pdf", width = 9, height = 3)       # Export PDF
grid.table(degra.sr.mod.allsums)
dev.off()
...
degra.sr.mod.plot<-plot_grid(
photo.sr.plot+ theme(legend.position = "none"),
microbe.sr.plot+ theme(legend.position = "none"),
total.sr.plot+ theme(legend.position = "none"), extract.legend,
  rel_widths = c(8,8,8,3), ncol=4)

degra.sr.mod.plot

ggsave("figures/degra.sr.mod.plot.pdf", height=4, width=12)

```
compile raw plots and model-diff plots for final figures
```{r DOC model plots compiled}

all.degra.mod.plots<-plot_grid(
degra.mod.plot+ theme(legend.position = "none"),
degra.suva.mod.plot+ theme(legend.position = "none"),
degra.hix.mod.plot+ theme(legend.position = "none"), nrow=3, rel_widths = c(8,8,8), labels = "AUTO")

all.degra.mod.plots

ggsave("figures/all.degra.mod.plots.pdf", height=12, width=12)

```

Comnine AIC and Mod sums for degradation
```{r}
library(gridExtra)
#merge all the dataframes
all.degra.mod.sums=rbind(degra.mod.allsums,degra.suva.mod.allsums,degra.hix.mod.allsums)

#convert dataframe into a table for export
pdf("/Users/Cody1/Desktop/GitHub/Pyromania2/output/Codys.tables/all.degra.mod.sums.pdf", width = 8.5, height = 7.8)       # Export PDF
grid.table(all.degra.mod.sums, rows=NULL)
dev.off()

#merge all the dataframes
all.degra.mod.AIC=rbind(AIC.degra.mod,AIC.degra.suva.mod,AIC.degra.hix.mod)

#convert dataframe into a table for export
pdf("/Users/Cody1/Desktop/GitHub/Pyromania2/output/Codys.tables/all.degra.mod.AIC.pdf", width = 6.5, height = 7.8)       # Export PDF
grid.table(all.degra.mod.AIC, rows=NULL)
dev.off()
...
Amend numeric gradient to categorical factors (low/med/high plant biomass) for ANOVA

```{r}

#make a copy of existing DOC dataframe
DOC.df2=data.frame(DOC.df)
DOC.df2=DOC.df2[-(1:32),]

#add categorical low/med/high biomass
DOC.df2$plant.level=as.factor(ifelse(DOC.df2$plant.mass..g<=75, "Low",
  ifelse (DOC.df2$plant.mass..g>=225, "High", "Medium"))) 
#calculate group mean                                    
DOC.df2$mean=ave(DOC.df2$DOC..mg.L, DOC.df2$plant.level)  
#reorder levels
DOC.df2$plant.level <- factor(DOC.df2$plant.level, levels = c("Low", "Medium","High"))
levels(DOC.df2$plant.level)

#new plot format
Fig.formatting.boxplot<-(theme_classic()) +
  theme(text=element_text(size=14),
        legend.text.align = 0,
        panel.border = element_rect(fill=NA, colour = "black", size=1),
        aspect.ratio=1, 
  ...
  theme(legend.key.size = unit(0.4, "cm")) +
  theme(aspect.ratio=1.3) +
  theme(panel.spacing=unit(c(0), "pt"))
...
p=ggplot(DOC.df2, aes(x=plant.level, y=sr, fill=Treatment)) + 
  geom_boxplot()+
  facet_grid(~Time.point,  labeller = as_labeller(plot.mean_labels))+
  Fig.formatting.boxplot+
  scale_color_manual(values = c("brown1", "mediumseagreen")) + 
  scale_fill_manual(values = c("brown1", "mediumseagreen")) +
  ylab("DOC (mg/L)") +
  xlab("plant biomass categories")

p 

DOC.df2 %>%
  ggplot( aes(x=plant.level, y=hix, fill=Treatment)) +
    geom_boxplot() +
    theme(
...
library("ggpubr")
ggline(DOC.df2, x = "plant.level", y = "DOC..mg.L", 
       add = c("mean_se", "jitter"), 
       order = c("Low", "Medium", "High"),
       ylab = "DOC (mg/L)", xlab = "Category")
...
DOC.df2_T1=data.frame(DOC.df2[DOC.df2$Time.point=="T1",])
DOC.df2_T2=data.frame(DOC.df2[DOC.df2$Time.point=="T2",])
DOC.df2_T3=data.frame(DOC.df2[DOC.df2$Time.point=="T3",])
DOC.df2_T4=data.frame(DOC.df2[DOC.df2$Time.point=="T4",])

# Compute the analysis of variance Time 1
aov.m1_T1.low<- aov(DOC..mg.L ~ Treatment, subset=plant.level=="Low" , data=DOC.df2_T1)
aov.m1_T1.med<- aov(DOC..mg.L ~ Treatment, subset=plant.level=="Medium" , data=DOC.df2_T1)
aov.m1_T1.high<- aov(DOC..mg.L ~ Treatment, subset=plant.level=="High" , data=DOC.df2_T1)
# Summary of the analysis
summary(aov.m1_T1.high)
plot(aov.m1_T1.med)

# Compute the analysis of variance Time 2
aov.m1_T2.low<- aov(DOC..mg.L ~ Treatment, subset=plant.level=="Low" , data=DOC.df2_T2)
aov.m1_T2.med<- aov(DOC..mg.L ~ Treatment, subset=plant.level=="Medium" , data=DOC.df2_T2)
aov.m1_T2.high<- aov(DOC..mg.L ~ Treatment, subset=plant.level=="High" , data=DOC.df2_T2)
# Summary of the analysis
summary(aov.m1_T2.med)
summary(aov.m1_T2.med)
summary(aov.m1_T2.high)
plot(aov.m1_T2.med)


# Compute the analysis of variance Time 3
aov.m1_T3.low<- aov(DOC..mg.L ~ Treatment, subset=plant.level=="Low" , data=DOC.df2_T3)
aov.m1_T3.med<- aov(DOC..mg.L ~ Treatment, subset=plant.level=="Medium" , data=DOC.df2_T3)
aov.m1_T3.high<- aov(DOC..mg.L ~ Treatment, subset=plant.level=="High" , data=DOC.df2_T3)
# Summary of the analysis
summary(aov.m1_T3.high)
plot(aov.m1_T1.med)


# Compute the analysis of variance Time 4
aov.m1_T4.low<- aov(DOC..mg.L ~ Treatment, subset=plant.level=="Low" , data=DOC.df2_T4)
aov.m1_T4.med<- aov(DOC..mg.L ~ Treatment, subset=plant.level=="Medium" , data=DOC.df2_T4)
aov.m1_T4.high<- aov(DOC..mg.L ~ Treatment, subset=plant.level=="High" , data=DOC.df2_T4)
# Summary of the analysis
summary(aov.m1_T4.high)
plot(aov.m1_T4.high)
