...
DOC model plot 
```{r DOC model plot, fig.dim=c(10,3)}
...
DOC diff plot
```{r DOC diff plot, fig.dim=c(10,3)}
...
Import fluorescence and UV-vis absorbance index files. Begin with SUVA.
...
Import the rest of the index files
...
SUVA model plot
```{r SUVA model plots, fig.dim=c(10,3)}
extract.legend <- get_legend(
  # create some space to the left of the legend
  DOC.T1.mod.plot + theme(legend.box.margin = margin(0, 0, 0, 10)))
...
SUVA diff plot
```{r SUVA diff plots, fig.dim=c(10,3)}
...
Sr model plot
```{r Sr model plots, fig.dim=c(10,3)}
...
Sr diff plot
```{r Sr diff plots, fig.dim=c(10,3)}
...
HIX model plot
```{r HIX model plot, fig.dim=c(10,3)}
...
HIX diff plot
```{r HIX diff plot, fig.dim=c(10,3)}
...
Fr model plot
```{r Freshness model plots, fig.dim=c(10,3)}
...
Fr diff plot
```{r Freshness diff plots, fig.dim=c(10,3)}
...
Fl model plot
```{r Fl model plots, fig.dim=c(10,3)}
...
Fl diff plot
```{r Fl diff plots, fig.dim=c(10,3)}
...
```{r All indices model plots, fig.show='hide', message=FALSE, collapse=TRUE, results='hide'}
...
UVBIO effect sizes
```{r UVBIO effect size, fig.show='hide', message=FALSE, collapse=TRUE, results='hide'}
library(effectsize)
...
UVBIO model plot
```{r UVBIO model plots, fig.dim=c(10,3)}
...
Sage and willow model plot
```{r Sage and willow model plot, fig.dim=c(10,3)}
...
Sage and willow model plot
```{r Sage and Willow plots, fig.dim=c(10,3)}
...
Combined sage and willow plots
```{r Sage and willow model plots combined, fig.dim=c(10,3)}
