...
author: "CJ Spiegel et al. 2023, in review Global Change Biology"
date: "08/01/2023"
output:
  html_document:
    code_folding: hide
...
```{r DOC analysis and individual plots, fig.show='hide', message=FALSE, collapse=TRUE, results='hide'}
######## T0 model
...
Add these new metrics to DOC df
...
```{r SUVA analysis and plots, fig.show='hide', message=FALSE, collapse=TRUE, results='hide'}
######## T0 model
...
Generate dataframe for model outputs
```{r results='hide', fig.show='hide', message =FALSE, collapse=TRUE}
####Parametric output
...
```{r SUVA model plots compiled, results='hide', fig.show='hide', message =FALSE, collapse=TRUE}
...
```{r sr model plots compiled, results='hide', fig.show='hide', message =FALSE, collapse=TRUE}
...
```{r HIX analysis and plots, results='hide', fig.show='hide', message =FALSE, collapse=TRUE}
######## T0 model
...
```{r Freshness analysis and plots, results='hide', fig.show='hide', message =FALSE, collapse=TRUE}
######## T0 model
...
```{r All indices figure, results='hide', fig.show='hide', message =FALSE, collapse=TRUE}
...
r_squared_microbes
r_squared_total
# Assuming you have already fitted your GAM model m1.photo.perc

# Create a range of values for the predictor of interest (plant.mass..g)
plant.mass.values <- c(0, 5, 10, 25, 50, 75, 100, 125, 150, 175, 200, 225, 250, 300, 400)
effect_sizes <- numeric(length(plant.mass.values))

# Generate partial effect predictions for the UV-only microbes treatment
# Loop through the specific plant.mass..g values
for (i in 1:length(plant.mass.values)) {
  # Generate partial effect prediction for the UV-only microbes treatment at each plant.mass..g value
  uv_microbes_effect <- predict(m1.photo.perc, newdata = data.frame(Treatment = "UV-only microbes", plant.mass..g = plant.mass.values[i]), type = "link")
  m1.microbe.perc
  # Calculate the effect size by taking the difference in predicted values between "UV-only microbes" and a reference group
  effect_size <- uv_microbes_effect$fit - predict(m1.total.perc, newdata = data.frame(Treatment = "microbes-only", plant.mass..g = plant.mass.values[i]), type = "link")
  
  # Store the effect size in the vector
  effect_sizes[i] <- effect_size
}
# Calculate the standard errors for the predictions
se_uv_microbes_effect <- sqrt(uv_microbes_effect$se.fit^2)

# Calculate the effect size by taking the difference in predicted values
# between "UV-only microbes" and a reference group (e.g., "microbes-only")
effect_size <- uv_microbes_effect$fit - predict(m1.photo.perc, newdata = data.frame(Treatment = "microbes-only", plant.mass..g = plant.mass.range), type = "link")

# Plot the partial effect with confidence intervals
ggplot(data = data.frame(plant.mass..g = plant.mass.range, effect_size = effect_size, se = se_uv_microbes_effect)) +
  geom_line(aes(x = plant.mass..g, y = effect_size), color = "blue", size = 1) +
  geom_ribbon(aes(x = plant.mass..g, ymin = effect_size - 1.96 * se, ymax = effect_size + 1.96 * se), fill = "blue", alpha = 0.2) +
  labs(x = "Plant Mass (g)", y = "Effect Size") +
  ggtitle("Effect Size of UV-only Microbes Treatment") +
  theme_minimal()
```
...
```{r Sage and Willow analysis and plots, results='hide', fig.show='hide', message =FALSE, collapse=TRUE}
#load file
...
compile raw plots and model-diff plots for final figures
```{r model plots compiled, results='hide', fig.show='hide', message =FALSE, collapse=TRUE}
