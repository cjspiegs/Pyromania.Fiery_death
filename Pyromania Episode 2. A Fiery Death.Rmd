---
title: "Pyromania—Life after a fiery death: fire and plant biomass loading affect dissolved organic matter in experimental ponds"
author: "CJ Spiegel et al. 2023, Global Change Biology"
date: "11/06/2023"
output:
  html_document:
    code_folding: hide
...
![**Figure**. Images showing (**a**) controlled burning of plant material, (**b**) example of low severity burn (**c**) packed plant material in mesh bags, (**d**) experimental pond set up, (**e**) plant material added to ponds, and (**f**) in situ incubations.](figures/Fig S1 design.png){width=80%}
...
* Analyze DOC at each sampling time point.  
* Run model selection and produce plots for each individual timepoint
* Graph the timepoints as a 5 panel figure.  

```{r DOC import, collapse=TRUE, result='hide'}
...
* First, run GAMs to see what is happening and where differences lie.  
* We will then make the five panel figure
```{r DOC analysis, fig.show='hide', message=FALSE, collapse=TRUE, results='hide'}
######## T0 model
...
```{r DOC output, fig.show='hide', message=FALSE, collapse=TRUE, results='hide' }
####Parametric output
...
```{r DOC diff plot, fig.dim=c(13,5), fig.cap="**Figure**. Differences in dissolved organic carbon (DOC) concentration for fitted smooth functions between treatments (burned vs. unburned) across plant biomass. Difference in trends shown in red with approximate 95% confidence intervals."}
...
### Import all UVBIO files
Import the data for UVBIO (data from the in situ incubations) and run a loop to clean up all files. This creates a single dataframe (df) by taking the raw data files, aligning metadata, and filtering to make a clean df for GAMs. 

* Later, we will analyze % DOC change from the incubations at Day 10.  
* Run model selection and produce plots for each degradation pathway (UV-only, Microbes-only, UV and Microbes).
* Plot the UVBIO data.

```{r UVBIO, fig.show='hide', message=FALSE, collapse=TRUE, results='hide'}
##### grab files in a list
...
### Import fluorescence and UV-vis absorbance index files. 
...
*We will analyze fluorescence and UV-vis absorbance across the biomass gradient at each sampling time point.
* Run model selection and produce plots for each index.
...
```{r SUVA all times, fig.show='hide', message=FALSE, collapse=TRUE, results='hide'}
###### SUVA Time point 0 
...
### Import the rest of the fluorescence and UV-vis absorbance data
...
```{r other indicies, fig.show='hide', message=FALSE, collapse=TRUE, results='hide'}
# Time 0
...
#### Add the fluroescence and UV-vis absorbance data back to original DOC df
Now we will have all of our data in one single df.

```{r new DOC df, fig.show='hide', message=FALSE, collapse=TRUE, results='hide'}
...
* First, run GAMs to see what is happening and where differences lie.  
* We will then make a plot for SUVA.

```{r SUVA analysis, fig.show='hide', message=FALSE, collapse=TRUE, results='hide'}
######## T0 model
...
```{r SUVA output, fig.show='hide', message=FALSE, collapse=TRUE, results='hide'}
####Parametric output
...
```{r SUVA diff plots, fig.dim=c(13,5), fig.cap="**Figure**. Differences between fitted smooth functions for the fire treatments (difference in trends; solid lines) for fluorescence and UV-vis absorbance indices shown in red with approximate 95% confidence intervals."}
...
* First, run GAMs to see what is happening and where differences lie.  
* We will then make a plot for Sr.

```{r Sr analysis, fig.show='hide', message=FALSE, collapse=TRUE, results='hide'}
######## T0 model
...
```{r Sr output, fig.show='hide', message=FALSE, collapse=TRUE, results='hide'}
####Parametric output
...
```{r Sr model plots, fig.dim=c(13,5), fig.cap="**Figure**. Changes in Sr with increasing plant biomass for the burned (red) and unburned (green) treatments.  Each dot represents a different pond. Plots with a single smooth (global) indicate that a model with one fit line for the burned and unburned treatments best describes the data, while separate smooths indicate that a model with different fits for the two treatments showed best agreement."}
...
```{r Sr diff plots, fig.dim=c(13,5), fig.cap="**Figure**. Differences between fitted smooth functions for the fire treatments (difference in trends; solid lines) for fluorescence and UV-vis absorbance indices shown in red with approximate 95% confidence intervals."}
...
* First, run GAMs to see what is happening and where differences lie.  
* We will then make a plot for HIX.

```{r HIX analysis, fig.show='hide', message=FALSE, collapse=TRUE, results='hide'}
######## T0 model
...
```{r HIX output, fig.show='hide', message=FALSE, collapse=TRUE, results='hide'}
####Parametric output
...
```{r HIX model plot, fig.dim=c(13,5), fig.cap= "**Figure**. Changes in HIX with increasing plant biomass for the burned (red) and unburned (green) treatments.  Each dot represents a different pond. Plots with a single smooth (global) indicate that a model with one fit line for the burned and unburned treatments best describes the data, while separate smooths indicate that a model with different fits for the two treatments showed best agreement."}
...
```{r HIX diff plot, fig.dim=c(13,5), fig.cap= "**Figure**. Differences between fitted smooth functions for the fire treatments (difference in trends; solid lines) for fluorescence and UV-vis absorbance indices shown in red with approximate 95% confidence intervals."}
...
#### Freshness (BIX)  analysis
We can model how HIX changes across the biomass gradient and over time.

* First, run GAMs to see what is happening and where differences lie.  
* We will then make a plot for BIX.
```{r Freshness analysis, fig.show='hide', message=FALSE, collapse=TRUE, results='hide'}
######## T0 model
...
#### BIX output
This generates a table with GAM summaries.
```{r Freshness output, fig.show='hide', message=FALSE, collapse=TRUE, results='hide'}
####Parametric output
...
```{r Freshness model plots, fig.dim=c(13,5), fig.cap= "**Figure**. Changes in BIX with increasing plant biomass for the burned (red) and unburned (green) treatments.  Each dot represents a different pond. Plots with a single smooth (global) indicate that a model with one fit line for the burned and unburned treatments best describes the data, while separate smooths indicate that a model with different fits for the two treatments showed best agreement."}
...
#### BIX diff plot
```{r Freshness diff plots, fig.dim=c(13,5), fig.cap= "**Figure**. Differences between fitted smooth functions for the fire treatments (difference in trends; solid lines) for fluorescence and UV-vis absorbance indices shown in red with approximate 95% confidence intervals."}
...
* First, run GAMs to see what is happening and where differences lie.  
* We will then make a plot for Fl.

```{r Fl analysis, fig.show='hide', message=FALSE, collapse=TRUE, results='hide'}
library(mgcv)
library(tidymv)
######## T0 model
...
```{r Fl output, fig.show='hide', message=FALSE, collapse=TRUE, results='hide'}
####Parametric output
...
```{r Fl model plots, fig.dim=c(13,5), fig.cap= "**Figure**. Changes in Fl with increasing plant biomass for the burned (red) and unburned (green) treatments.  Each dot represents a different pond. Plots with a single smooth (global) indicate that a model with one fit line for the burned and unburned treatments best describes the data, while separate smooths indicate that a model with different fits for the two treatments showed best agreement."}
...
```{r Fl diff plots, fig.dim=c(13,5), fig.cap= "**Figure**. Differences between fitted smooth functions for the fire treatments (difference in trends; solid lines) for fluorescence and UV-vis absorbance indices shown in red with approximate 95% confidence intervals."}
...
### All indices model plot
This is a large 5 x 4 figure with all fluorescence and UV-vis absorbance indices across sampling time points.

```{r All indices model plots, fig.dim=c(13,18), fig.cap= "**Figure**. Changes in a: SUVA, b: spectral slope ratio, c: humification index, d: freshness index, and e: fluorescence index with increasing plant biomass for the burned (red) and unburned (green) treatments.  Each dot represents a different pond. Plots with a single smooth (global) indicate that a model with one fit line for the burned and unburned treatments best describes the data, while separate smooths indicate that a model with different fits for the two treatments showed best agreement."}
...
### All indices diff plot

```{r All indices diff plots, fig.dim=c(13,18), fig.cap= "**Figure**.Differences between fitted smooth functions for the treatments (difference in trends; solid lines) for fluorescence and UV-vis absorbance indices shown in red with approximate 95% confidence intervals."}
...
### Create UVBIO df
Since we initially imported the UVBIO files, we can now create a unqiue df for each degradation pathway and merge them: photo.df = UV-only, microbes.df= microbes-only, total.df= UV and microbes. The degra.df now contains all 3 pathways.

```{r UVBIO import, results='hide', fig.show='hide', message =FALSE, collapse=TRUE}
#+UV/+Bio=A
#-UV/+Bio=B
#+UV/-Bio=C
...
* First, run GAMs to see what is happening and where differences lie.  
* We will then make a plot for UVBIO.

```{r UVBIO analysis, fig.show='hide', message=FALSE, collapse=TRUE, results='hide'}
library(mgcv)
...
#### UVBIO output
This generates a table with GAM summaries.
```{r UVBIO output, fig.show='hide', message=FALSE, collapse=TRUE, results='hide'}
####Parametric output
...
#### UVBIO model plot
```{r UVBIO model plots, fig.dim=c(13,5), fig.cap= "**Figure**. Dissolved organic carbon (DOC) loss as percent change for burned (red) and unburned (green) treatments across plant biomass (g).  Each point represents a different pond. Plots with a single smooth (global) indicate that a model with one fit line for the burned and unburned treatments best describes the data, while separate smooths indicate that a model with different fits for the two treatments showed best agreement."}
...
### Import and analyze sage and willow decomposition data
...
* First, run GAMs to see what is happening and where differences lie.  
* We will then make a plot for sage and willow within and between treatments.
  
```{r Sage and willow analysis, fig.show='hide', message=FALSE, collapse=TRUE, results='hide'}
#load file
...
#### Sage and willow output
This generates a table with GAM summaries.

```{r Sage and willow between output, fig.show='hide', message=FALSE, collapse=TRUE, results='hide'}
####Parametric output
...
#### Sage and willow within treatment analysis
...
```{r Sage and willow treatment analysis, fig.show='hide', message=FALSE, collapse=TRUE, results='hide'}
...
#### Sage and willow within treatment output
This generates a table with GAM summaries.
```{r Sage and willow within output, fig.show='hide', message=FALSE, collapse=TRUE, results='hide'}
####Parametric output
...
#### Sage and willow combined output
This combines to outputs into a single table with GAM summaries.

```{r Sage and Willow combined output, results='hide', fig.show='hide', message =FALSE, collapse=TRUE}
library(gridExtra)
...
#### Sage and willow combined plot
We can combine the two decomposition plots into one so that we now have decomposition between and within fire treatment.

```{r Sage and Willow plots, fig.dim=c(13,10), fig.cap= "**Figure**. Sage and willow percent mass loss across the plant biomass gradient at the end of 89 days. Mass loss between sage (purple) and willow (brown) within treatments. Mass loss for sage and willow between burned (red) and unburned (green) treatments. Each point represents a different pond. Plots with a single global smooth indicate that a model with one fit line for the burned and unburned treatments best describes the data, while separate smooths indicate that a model with different fits for the two treatments showed best agreement."}
